...
pfa <- function(Z, Sigma, t, Kmax, reg="L1", e=.05, gamma, K, plot="-log") {
  #options(digits=22); options(scipen = -3)
  Z <- as.vector(Z)###############
  ...
  lambda <- pca$d
  eigvec <- pca$v
  # pca <- irlba(Sigma, nv = Kmax, symmetric = TRUE)
  # lambda <- pca$d
  # eigvec <- pca$v
  ...
  # factor adjusted procedure
   ...
   adj.P <- statistics <- as.vector(rep(0,p))
   for (j in 1:p) {
        if (inv_a[j]>0)  {
           adj.P[j] <- 2*(pnorm(-abs(Z[j] - bW.est[j])/inv_a[j]))
           statistics[j] <- (Z[j] - bW.est[j])/inv_a[j]
           #??统??量????/inv_a[j]????mean diff去??/inv_a[j]????要注??/sqrt(96/2))
        }  else  {
          stop("inv_a < 0")
          # if (abs(Z[j]-bW.est[j])==0){
          #   adj.P[j] <- 1
          #   } else {
          #     adj.P[j] <- 2*( 1- pnorm(abs(Z[j] - bW.est[j])/10^(-6)))
          #   }
        }
   }
  ...
  statistics <- data.frame(statistics = statistics)
  if (t.default)   {
       FDPvals <- data.frame( minus.logt= - log(t), rejects= Rt, false.rejects=Vt, FDP=FDPt )
  } else {
       FDPvals <- data.frame( t= t, rejects= Rt, false.rejects=Vt, FDP=FDPt, pvaluess = P1)##########################################
  }
  results <- list("Pvalue"=Pvals, "adjPvalue"=adjPvals, "statistics"=statistics,
                  "FDP"=FDPvals, "pi0"=p0.est/p, "K"=K, "sigma"=NULL)
