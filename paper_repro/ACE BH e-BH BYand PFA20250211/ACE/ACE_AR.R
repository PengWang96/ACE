...
clusterExport(cl, c("K", "v", "gama", "sqrt_factor", "p", "sigmaa"))  # 导出全局变量
...
simulation_task <- function(params, iter) {
  tryCatch({
    ...
    error_matrix <- t(mvrnorm(n, rep(0,p), sigmaa))
...
print(final_results)
write.csv(final_results, "./output/BY_AR.csv", row.names = FALSE)
t2 <- Sys.time()
(t2-t1)