...
simulation_task <- function(params) {
  tryCatch({
    ...
    generate_X <- function() {
      ...
      error_matrix <- t(rmvt(n1, sigma = diag(p), df = 4)) ###################
      # error_matrix <- t(rmvt(n1, sigma = diag(p), df = 10)) ##################
...
# 输出结果
print(final_results)
write.csv(final_results, "./output/BY_two_t4_AR.csv", row.names = FALSE)
# write.csv(final_results, "./output/BY_two_t10_AR.csv", row.names = FALSE)
t2 <- Sys.time()
(t2 - t1)