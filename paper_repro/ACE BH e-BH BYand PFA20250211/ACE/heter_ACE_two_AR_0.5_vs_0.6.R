...
simulation_task <- function(params) {
  tryCatch({
    ...
    generate_X <- function() {
      ...
      sigmaa <- sigmaa - diag(rep(1,p)) + diag(c(2 + mu - mu2))
      diag(sigmaa[index0,index0]) <- (runif(length(index0), min=1, max=1))
      error_matrix <- t(mvrnorm(n1, rep(0,p), sigmaa))
...
# 输出结果
print(final_results)
write.csv(final_results, "./output/heter_BY_two_AR_0.5_vs_0.6.csv", row.names = FALSE)
t2 <- Sys.time()
(t2-t1)