...
# 并行初始化 ----------------------------------------------------------
numCores <- detectCores() - 2  # 保留2个核心给系统
...
simulation_task <- function(params, iter) {
  tryCatch({
    ...
    error_matrix <- t(rmvt(n, sigma = diag(p), df = 4))
    # error_matrix <- t(rmvt(n, sigma = diag(p), df = 10)) #####################
    
    # 向量化计算替代循环
    X <- mu %*% rep(1, n) + B %*% f_matrix + error_matrix
    
    RE <- ACE(X, H0_indicator = berlii, gama = gama)
...
print(final_results)
write.csv(final_results, "./output/BY_t4.csv", row.names = FALSE)
# write.csv(final_results, "./output/BY_t10.csv", row.names = FALSE)
t2 <- Sys.time()
(t2-t1)