rm(list = ls())
setwd("D:/R/factor model20230625/BH and PFA2023_4_25/BH1")
t1 <- Sys.time()
p <- 2000; n <- 600 ; K <- 3; lb <- 0.1; ub <- 0.4; v <- 4; sqrt_factor <- sqrt(v / (v - 2))
# p <- 2000; n <- 50 ; K <- 3; lb <- 0.3; ub <- 0.6
PP <- 0.2 #true pi_1
gama <- 0.05 #control level
iteraton <- 5 # number of repeat
library(MASS)
...
for (jj in 1:iteraton){
  ...
  Z <- colMeans(X)*sqrt(nrow(X))/sqrt(diag(SigmaX))
