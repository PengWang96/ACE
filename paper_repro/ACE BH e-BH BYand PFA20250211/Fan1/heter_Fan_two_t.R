...
for (jj in 1:iteraton){
  ...
  sigmaa <- diag(c(2 + mu - mu2)); sigmaa2 <- diag(c(2.5 + mu - mu2))
  diag(sigmaa[index0,index0]) <- (runif(length(index0), min=1, max=1))
  diag(sigmaa2[index0,index0]) <- (runif(length(index0), min=1, max=1))
  mydata1 <- (rmvt(n1, sigma = sigmaa,  df = 10))
  mydata2 <- (rmvt(n2, sigma = sigmaa2, df = 10))
  
  # sigmaa <- diag(c(2 + mu - mu2)); sigmaa2 <- diag(c(2.5 + mu - mu2))
  # diag(sigmaa[index0,index0]) <- (runif(length(index0), min=1, max=1))
  # diag(sigmaa2[index0,index0]) <- (runif(length(index0), min=1, max=1))
  # mydata1 <- (rmvt(n1, sigma = sigmaa, df = 4))
  # mydata2 <- (rmvt(n2, sigma = sigmaa2, df = 10))
