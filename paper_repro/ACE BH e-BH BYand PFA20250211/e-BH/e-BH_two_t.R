...
source("e-BH.R")
# 设定并行计算的线程数
numCores <- detectCores() - 10  # 获取可用的CPU核心数，减1以保留一个核心给操作系统
...
results <- foreach(jj = 1:iteraton, .combine = 'rbind', .packages = c('MASS', 'mvtnorm')) %dopar% {
  ...
  # t_error <- t(rmvt(n1, sigma = diag(p), df = 4))
  # t_error2 <- t(rmvt(n2, sigma = diag(p), df = 4))
  
  # t_error <- t(rmvt(n1, sigma = diag(p), df = 4))
  # t_error2 <- t(rmvt(n2, sigma = diag(p), df = 10))
  
  t_error <- t(rmvt(n1, sigma = diag(p), df = 10))
  t_error2 <- t(rmvt(n2, sigma = 1.5*diag(p), df = 10))
  
  for (i in 1:n1){
    f <- t(rmvt(1, sigma = diag(K1), df = v))/sqrt_factor
    X[,i] <- mu + B %*% f + t_error[,i]########
  }
  
  for (i in 1:n2){
    f2 <- t(rmvt(1, sigma = diag(K2), df = v))/sqrt_factor
